...
from src.trainer.mocpo_trainer import MOCPOTrainer
...
class ScriptArguments:
    ...
    margin_reward_model_name_1: str = field(metadata={"help": "the margin reward model name of xcomet"})
    margin_reward_model_name_2: str = field(metadata={"help": "the margin reward model name of kiwi"})
    ...
    w1: Optional[float] = field(default=1/3, metadata={"help": "weight"})
    w2: Optional[float] = field(default=1/3, metadata={"help": "weight"})
...
training_args = TrainingArguments(
    output_dir="./output/PKU-Alignment/PKU-SafeRLHF-10K/mocpo/lm/0.3xcomet+0.3kiwi+0.3fluency",
    overwrite_output_dir=True,
    seed=42,
    ...
    fp16=True,
    remove_unused_columns=False,
    run_name="PKU-Alignment/PKU-SafeRLHF-10K/mocpo/lm/0.3xcomet+0.3kiwi+0.3fluency",
    report_to="wandb",
...
# peft
model = prepare_model_for_peft(sft_model, peft_config=script_args.peft_config, args=training_args)
# load frozon margin reward weights as lora
model.load_adapter(script_args.margin_reward_model_name_1, adapter_name="margin_reward_xcomet")
model.load_adapter(script_args.margin_reward_model_name_2, adapter_name="margin_reward_kiwi")
...
trainer.set_wrapped_margin_reward_model_list(
    RewardWrapperList([
        ImplicitRewardWrapper(
            model=PeftAsPreTrained(trainer.model, "margin_reward_xcomet"),
        ...
        ImplicitRewardWrapper(
            model=PeftAsPreTrained(trainer.model, "margin_reward_kiwi"),
            ref_model=PeftAsPreTrained(trainer.model), 
            tokenizer=tokenizer, 
            beta=script_args.beta,
            prompt_template=script_args.prompt_template,
    ...
    w=(1-script_args.w1-script_args.w2, script_args.w1, script_args.w2),
