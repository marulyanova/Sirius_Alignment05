...
@dataclass
class MOCPODataMapFunc(DPODataMapFunc):
...
@dataclass
class MOCPODataCollatorWithPadding(DPODataCollatorWithPadding):
...
class MOCPOTrainer(DPOTrainer):
    """
    The MOCPOTrainer is a light-weight extension of CPOTrainer that supports training with 
    ...
    def __init__(
        ...
        if tokenize_map_func is None:
            tokenize_map_func = MOCPODataMapFunc(tokenizer)

        if data_collator is None:
            data_collator = MOCPODataCollatorWithPadding(tokenizer)
    ...
    def set_wrapped_margin_reward_model_list(
        ...
        Args:
            wrapped_margin_reward_model_list (`src.utils.RewardWrapperList`):
                A list of reward model to act as margin in `moCPO_loss`. 
            w (`List[float]`):
            ...
            prepare (`bool`):
                ...
                See `scripts/moCPO/beavertails/moCPO.py` for a complete example.
        """
        if prepare:
    ...
    def mocpo_loss(
        ...
        chosen_rewards   = (1/self.w[0])*(self.beta * (policy_chosen_logps)   - chosen_margin_reward   @ self.w[1:])
        rejected_rewards = (1/self.w[0])*(self.beta * (policy_rejected_logps) - rejected_margin_reward @ self.w[1:])

        logits = chosen_rewards - rejected_rewards
        if self.loss_type == "sigmoid":
            losses = -F.logsigmoid(logits) - (policy_chosen_logps)
        elif self.loss_type == "hinge":
            losses = torch.relu(1 - logits) - (policy_chosen_logps)
        else:
    ...
    def dpo_loss(self, *args, **kwargs):
        """Disable the `cpo_loss` inherited from the DPOTrainer"""
    ...
    def get_batch_metrics(
        ...
        losses, chosen_rewards, rejected_rewards = self.mocpo_loss(
            policy_chosen_logps,
            policy_rejected_logps,
            reference_chosen_logps,
            reference_rejected_logps,
