...
def preprocess_cpo_data(train_raw_data, valid_raw_data, test_raw_data, pairs, tokenizer, shots_eval_dict, data_args, training_args, model_args, logger):
    ...
    def get_chosen_reject_adv(example, target_lang):
        sys1_score_key = f"gpt4_{target_lang}_{data_args.cpo_scorer}"
        sys2_score_key = f"alma_{target_lang}_{data_args.cpo_scorer}"
        flu1_score_key = f"gpt4_{target_lang}_fluency"
        flu2_score_key = f"alma_{target_lang}_fluency"
        ref_flu_key = f"ref_{target_lang}_fluency"
        ref_score_key = f"ref_{target_lang}_{data_args.cpo_scorer}"
        ...
        # Defining the sentences and their scores
        sentences = [example[ref_output_key], example[sys1_output_key], example[sys2_output_key]]
        scores = [example[ref_score_key] * 2 / 3 + example[ref_flu_key] / 3,
                  example[sys1_score_key] * 2 / 3 + example[flu1_score_key] / 3,
                  example[sys2_score_key] * 2 / 3 + example[flu2_score_key] / 3]
    ...
    def cpo_prompt_function_adv(examples):
        new_examples = {
            "prompt": [],
            "chosen": [],
            "rejected": [],
        ...
        for ex in examples["translation"]:
            ...
            new_examples["prompt"].append(prompt)
            chosen, rejected = get_chosen_reject_adv(ex, target_lang)
            new_examples["chosen"].append(chosen)
            new_examples["rejected"].append(rejected)
        return new_examples
